  ...
  const html = `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>© ${new Date().getFullYear()} Thessah. All rights reserved.</p>
...
export async function sendTeamInviteEmail({ email, storeName, role, inviteLink }) {
  const subject = `You're invited to manage ${storeName} on Thessah`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; background: #f4f6fb; }
        .container { max-width: 600px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 16px rgba(40,116,240,0.07); }
        .header { background: #2874f0; color: white; padding: 32px 24px; text-align: center; }
        .header h1 { margin: 0 0 8px 0; font-size: 1.8rem; font-weight: 700; }
        .content { padding: 32px 24px; background: #f8f9fa; }
        .role-badge { display: inline-block; background: #fff; padding: 8px 16px; border-radius: 20px; border: 2px solid #3b82f6; color: #2563eb; font-weight: 600; margin: 16px 0; }
        .button { display: inline-block; background: #3b82f6; color: white; padding: 14px 36px; text-decoration: none; border-radius: 8px; margin: 24px 0; font-weight: bold; font-size: 1rem; }
        .button:hover { background: #2563eb; }
        .footer { text-align: center; margin-top: 30px; padding: 24px; color: #6b7280; font-size: 13px; background: #fff; }
        .note { background: #fef3c7; border-left: 4px solid #f59e0b; padding: 16px; border-radius: 4px; margin: 20px 0; font-size: 14px; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Join ${storeName} on Thessah</p>
        </div>
        <div class="content">
          <h2 style="margin-top:0;">You've been invited to collaborate</h2>
          <p>Someone has invited you to join their store on Thessah and help manage their products, orders, and business.</p>
          ...
          <div style="text-align: center;">
            ...
            <p style="font-size: 14px; color: #6b7280;">
              ${role === 'admin' ? 'You will have full access to all settings and team management.' : 'You will be able to create and edit products, view orders, and access analytics.'}
            </p>
          </div>
          ...
          <div class="note">
            <strong>⏰ Note:</strong> This invite will expire in 7 days. If you don't accept by then, ask the store owner to send a new invite.
          </div>

          <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
            <strong>Questions?</strong> Reply to this email or contact our support team at <a href="mailto:\${process.env.NEXT_PUBLIC_ADMIN_EMAIL}">\${process.env.NEXT_PUBLIC_ADMIN_EMAIL}</a>
          </p>
        </div>
        <div class="footer">
          <p>© ${new Date().getFullYear()} Thessah. All rights reserved.</p>
          <p>You're receiving this email because you were invited to join a store on Thessah.</p>
        </div>
      </div>
    </body>
    </html>
